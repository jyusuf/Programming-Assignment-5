...
int main(int argc, char *argv[]) {
  ...
  double *temp = malloc(sizeof(double)*(n/mpi_p));
  ...
  double aTwoStart = MPI_Wtime();

  MPI_Scatter(v, n/mpi_p, MPI_DOUBLE, temp, n/mpi_p, MPI_DOUBLE, 0, MPI_COMM_WORLD);

  merge_sort(n/mpi_p, temp);

  MPI_Gather(temp, n/mpi_p, MPI_DOUBLE, v, n/mpi_p, MPI_DOUBLE, 0, MPI_COMM_WORLD);

  double aTwoEnd = MPI_Wtime();

  printf("Start time %f, end time %f", aTwoStart, aTwoEnd);

  int step;
      int i;
      MPI_Scatter(v, count, MPI_DOUBLE, w, count, MPI_DOUBLE, 0, MPI_COMM_WORLD);


      if (mpi_p > 1) {
          for (step = 1, i = 0; step < mpi_p; step *= 2, i++) {
              MPI_Comm sec_comm;
              MPI_Comm_split(MPI_COMM_WORLD, color, mpi_rank % step, &sec_comm); //Half of the remaining processes send their data to the other half and no longer do any
              ///more work
              if (mpi_rank % step == 0) {
                  merge_sort(count * step, w); //The receiving processes merge their local data with the received data
              }
              MPI_Scatter(w, count, MPI_DOUBLE, v, count, MPI_DOUBLE, 0, sec_comm);

  if (mpi_rank == 0) {
	   merge_all(v, n, n/mpi_p);
	}
