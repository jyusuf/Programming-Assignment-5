...
int main(int argc, char *argv[]) {
        if (argc < 2) {
        	// () means optional
        	  ...
        	  printf("Starting time %f, ending time %f", aStart, aEnd);
        	  int count = n/mpi_p;
        	    int step;
        	        int i;
        	  int color = mpi_rank - (mpi_rank % step);
        	        MPI_Scatter(v, count, MPI_DOUBLE, temp, count, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        	        ...
        	        if (mpi_p > 1) {
        	            for (step = 1, i = 0; step < mpi_p; step *= 2, i++) {
        	                ...
        	                ///more work
        	                if (mpi_rank % step == 0) {
        	                    merge_sort(count * step, temp); //The receiving processes merge their local data with the received data
        	                }
        	                MPI_Scatter(v, count, MPI_DOUBLE, temp, count, MPI_DOUBLE, 0, sec_comm);
        	  }}
        	    if (mpi_rank == 0) {
